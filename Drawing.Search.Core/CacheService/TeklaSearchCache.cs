...
public class TeklaSearchCache : ISearchCache
{
    ...
    private static Tekla.Structures.Model.ModelObject GetRelatedModelObjectFromPart(Part part)
    {
        var model = new Model();
    ...
    public IEnumerable<object> GetRelatedObjects(string mainKey, string objectKey)
    {
        ...
        lock (_lockObject)
        {
            var keys = _relationshipsCache.TryGetValue(mainKey, out var relSet);
